...
class HotelController extends Controller
{
    ...
    public function store(Request $request, PackageImageStore $images)
    {
        ...
        if ($request->hasFile('logo')) {
            $data['logo'] = $images->store($request->file('logo'), $data['name'], 'hotels');
        }
    ...
    public function update(Request $request, Hotel $hotel, PackageImageStore $images)
    {
        $data = $this->validated($request, $hotel);
        $previousLogo = $hotel->logo;
        ...
        $hotel->update($data);
        if (array_key_exists('logo', $data) && $previousLogo !== $hotel->logo) {
